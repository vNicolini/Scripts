    ...
    if exist "%%i\" (
        ...
        for %%j in ("%%i\*") do (
            ...
            if not "%%~fj"=="" (
                ...
                "%MAKETX_PATH%" -v -u --threads 4 --format exr --checknan --constant-color-detect --opaque-detect --colorconvert "sRGB Encoded Rec.709 (sRGB)" ACEScg --unpremult --oiio  %%j
        ...
        rem Running maketx command to generate the .exr.tx file
        "%MAKETX_PATH%" -v -u --threads 4 --format exr --checknan --constant-color-detect --opaque-detect --colorconvert "sRGB Encoded Rec.709 (sRGB)" ACEScg --unpremult --oiio  %%i
