    ...
    if [ -d "$input" ]; then
        ...
        for file in "$input"/*; do
            ...
            if [ -f "$file" ]; then
                ...
                "$MAKETX" -v -u --threads 4 --format exr --checknan --constant-color-detect --opaque-detect --colorconfig "$OCIO" --colorconvert "Linear Rec.709 (sRGB)" ACEScg --unpremult --oiio "$file"
        ...
        # Running maketx command to generate the .exr.tx file
        "$MAKETX" -v -u --threads 4 --format exr --checknan --constant-color-detect --opaque-detect --colorconfig "$OCIO" --colorconvert "Linear Rec.709 (sRGB)" ACEScg --unpremult --oiio "$input"
