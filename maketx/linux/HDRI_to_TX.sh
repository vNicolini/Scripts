    ...
    if [ -d "$input" ]; then
        ...
        for file in "$input"/*; do
            ...
            if [ -f "$file" ]; then
                ...
                "$MAKETX" -v -u --envlatl --threads 4 --format exr -d float --checknan --nchannels 3 --constant-color-detect --opaque-detect --colorconfig "$OCIO" --colorconvert "Linear Rec.709 (sRGB)" ACEScg --unpremult --oiio "$file"
                ...
                # Construct the new name by appending "_Linear Rec.709 (sRGB)_ACEScg" to the base name, then appending the extension
                new_name="${base_name}_Linear Rec.709 (sRGB)_ACEScg.${file_ext}.tx"
        ...
        # Running maketx command to generate the .exr.tx file
        "$MAKETX" -v -u --envlatl --threads 4 --format exr -d float --checknan --nchannels 3 --constant-color-detect --opaque-detect --colorconfig "$OCIO" --colorconvert "Linear Rec.709 (sRGB)" ACEScg --unpremult --oiio "$input"
