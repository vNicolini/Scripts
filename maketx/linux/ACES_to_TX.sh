    ...
    if [ -d "$input" ]; then
        ...
        for file in "$input"/*; do
            ...
            if [ -f "$file" ]; then
                ...
                "$MAKETX" -v -u --threads 4 --format exr --fixnan box3 -constant-color-detect --monochrome-detect --opaque-detect --colorconfig "$OCIO" --unpremult --oiio "$file"
                ...
                # Construct the new name by appending "_raw" to the base name, then appending the extension
                new_name="${base_name}_raw.${file_ext}.tx"
        ...
        # Running maketx command to generate the .exr.tx file
        "$MAKETX" -v -u --threads 4 --format exr --fixnan box3 -constant-color-detect --monochrome-detect --opaque-detect --colorconfig "$OCIO" --unpremult --oiio "$input"
